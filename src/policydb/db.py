...
def _init_db_inner(conn: sqlite3.Connection, db_path: Path) -> None:
    ...
    if 85 not in applied:
        sql = (_MIGRATIONS_DIR / "085_backfill_location_addresses.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (85, "Backfill policy exposure addresses from assigned locations"),
        )
        conn.commit()

    if 86 not in applied:
        sql = (_MIGRATIONS_DIR / "086_client_exposures.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (86, "Add client_exposures table for annual exposure tracking"),
        )
        conn.commit()

    if 87 not in applied:
        sql = (_MIGRATIONS_DIR / "087_schematic_column.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
    ...
    if 89 not in applied:
        ...
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (89, "Add policy_exposure_links table and denominator column to client_exposures"),
        )
        conn.commit()
    ...
    if 94 not in applied:
        existing_cols = {r[1] for r in conn.execute("PRAGMA table_info(policies)").fetchall()}
        if "layer_notation" not in existing_cols:
            conn.execute("ALTER TABLE policies ADD COLUMN layer_notation TEXT")
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (94, "add layer_notation to policies"),
        )
        conn.commit()

    if 95 not in applied:
        sql = (_MIGRATIONS_DIR / "095_program_tower_coverage.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (95, "program_tower_coverage junction table"),
        )
        conn.commit()

    if 96 not in applied:
        sql = (_MIGRATIONS_DIR / "096_program_tower_lines.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (96, "program_tower_lines table"),
        )
        conn.commit()

    if 97 not in applied:
        existing_cols = {r[1] for r in conn.execute("PRAGMA table_info(policy_sub_coverages)").fetchall()}
        if "attachment_point" not in existing_cols:
            conn.execute("ALTER TABLE policy_sub_coverages ADD COLUMN attachment_point REAL")
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (97, "add attachment_point to policy_sub_coverages"),
        )
        conn.commit()
    ...
    if 104 not in applied:
        sql = (_MIGRATIONS_DIR / "104_issue_tracking.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (104, "Add issue tracking columns to activity_log"),
        )
        conn.commit()
        logger.info("Migration 104: added issue tracking columns to activity_log")

    if 105 not in applied:
        sql = (_MIGRATIONS_DIR / "105_escalation_dismissals.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (105, "Add escalation_dismissals table"),
        )
        conn.commit()
    ...
    if 116 not in applied:
        ...
        for col_name, col_def in [
            ...
            if col_name not in existing_cols:
                conn.execute(f"ALTER TABLE activity_log ADD COLUMN {col_name} {col_def}")
        # Backfill due_date on existing renewal issues
        conn.execute("""
            UPDATE activity_log
            SET due_date = (
            ...
            WHERE is_renewal_issue = 1 AND due_date IS NULL
        ...
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (116, "Add due_date and auto-close tracking columns"),
        )
        conn.commit()
    ...
    if 118 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "118_bound_date.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (118, "Add bound_date column to policies"),
        )
        conn.commit()
    ...
    if 122 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "122_outlook_sync.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (122, "Add outlook_message_id, source, email_snippet to activity_log"),
        )
        conn.commit()
    ...
    if 126 not in applied:
        sql = (_MIGRATIONS_DIR / "126_program_contacts.sql").read_text()
        conn.executescript(sql)
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (126, "Program contacts, milestones, and RFI program_uid"),
        )
        conn.commit()
        logger.info("Migration 126: created contact_program_assignments, program_milestones, added program_uid to client_request_bundles")

    if 127 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "127_team_chart_dismissals.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (127, "Track dismissed placement colleague suggestions on team charts"),
        )
        conn.commit()
    ...
    if 129 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "129_suggested_contacts.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (129, "Suggested contacts table for email sync contact capture"),
        )
        conn.commit()
        logger.info("Migration 129: created suggested_contacts table")

    if 130 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "130_issue_checklist.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (130, "Issue checklist table for freeform to-do items"),
        )
        conn.commit()
        logger.info("Migration 130: created issue_checklist table")
    ...
    if 132 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "132_email_from_to.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (132, "Add email_from and email_to to inbox and activity_log"),
        )
        conn.commit()
    ...
    if 147 not in applied:
        conn.executescript((_MIGRATIONS_DIR / "147_compliance_review_ux.sql").read_text())
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (147, "Compliance review UX: audit stamps on coverage_requirements + policies.endorsements"),
        )
        conn.commit()
    ...
    if 161 not in applied:
        conn.executescript(
            (_MIGRATIONS_DIR / "161_repair_misclassified_layers.sql").read_text()
        )
        repaired = conn.execute("SELECT changes()").fetchone()[0]
        conn.execute(
            "INSERT INTO schema_version (version, description) VALUES (?, ?)",
            (161, "Repair policies with layer_position='Primary' but attachment_point>0"),
        )
        conn.commit()
        logger.info("Migration 161: repaired %d misclassified excess policies", repaired)
...
def next_rfi_uid(conn: sqlite3.Connection, client_id: int) -> str:
    ...
    row = conn.execute(
        "SELECT rfi_uid FROM client_request_bundles WHERE client_id=? AND rfi_uid IS NOT NULL ORDER BY CAST(SUBSTR(rfi_uid, INSTR(rfi_uid, '-RFI') + 4) AS INTEGER) DESC LIMIT 1",
        (client_id,),
    ).fetchone()
    max_num = 0
