...
def policy_add(client_name):
    ...
    if click.confirm("Add optional fields now?", default=False):
        ...
        tower_group = colleague = uw_name = notes = ""
        is_standalone = False
        status = "Not Started"
...
@click.argument("client_name")
@click.option("--format", "fmt", default="markdown", type=click.Choice(["markdown", "csv", "json"], case_sensitive=False))
...
def export_schedule(client_name, fmt, save):
    ...
    from policydb.exporter import (
        export_schedule_csv, export_schedule_json, export_schedule_md, save_export
    )
    conn = _get_conn()
    client = _resolve_client(conn, client_name)
    client_id = client["id"]
...
def export_client(client_name, fmt, save):
    """Full client export."""
    from policydb.exporter import export_client_csv, export_client_json, export_client_md, save_export
    conn = _get_conn()
    client = _resolve_client(conn, client_name)
    client_id = client["id"]
...
def export_renewals(days, fmt, save):
    ...
    from policydb.exporter import export_renewals_csv, export_renewals_json, export_renewals_md, save_export
    conn = _get_conn()
    if fmt == "markdown":
