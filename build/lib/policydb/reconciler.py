...
_LEGAL_SUFFIX_RE = re.compile(
    ...
    r'Ltd\.?|Limited|Company|Group|Holdings?|Enterprises?|Associates?|'
    r'Partners?|Trust)\b[.,]?$',
    re.IGNORECASE,
)
...
def _fuzzy_match(ext_row: dict, candidates: list[dict]) -> tuple[dict | None, float]:
    ...
    for db in candidates:
        ...
        client_score = fuzz.WRatio(ext_client_norm, _normalize_client_name(db.get("client_name", "")))
        if client_score < 70:
            continue
    ...
    for i in list(remaining_ext):
        ...
        for db in candidates_15:
            ...
            if eff_delta <= 30 and exp_delta <= 30:
                ...
                if type_score >= 60 and type_score > best_score_15:
...
def _cross_pair_score(ext: dict, db: dict) -> float:
    ...
    # Policy number — strong signal when present (larger bonus than main fuzzy matcher)
    ext_pn = (ext.get("policy_number") or "").strip().upper()
    db_pn = (db.get("policy_number") or "").strip().upper()
    if ext_pn and db_pn:
        pn_score = fuzz.ratio(ext_pn, db_pn)
